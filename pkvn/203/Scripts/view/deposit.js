(function (define) {
    define(["jquery", "api", "translate", "utils", "QRCode"], function ($, api, translate, utils, QRCode) {
        ...
        var depositInFlight = false; // guards setDeposit against re-entry
        var gatewayInFlight = false; // guards submitGatewayDeposit across both network hops
        var rejectInFlight = false;  // guards reject request across both network hops
        var rejectShowTimer = null;  // pending timeout to reveal reject button after 5 min
        ...
        return function () {
            ...
            // TODO: remove - test QR download button
            window.__testGatewayQr = function () {
                document.getElementById("gatewayQrCode").innerHTML = "";
                new utils.QRCode(document.getElementById("gatewayQrCode"), {
                    text: "https://test.example.com/pay/12345", width: 200, height: 200, quietZone: 5
                });
                gatewayQrActive = true;
                $('#rowGatewayQr').show();
                $('#rowGatewaySubmit').hide();
                $('#rowGatewayCheck').show();
                $('#btnGatewayCheck').off('click').on('click', function () { alert('check status (test)'); });
                $('#btnGatewayDownloadQr').off('click').on('click', function () {
                    var canvas = document.querySelector('#gatewayQrCode canvas');
                    if (!canvas) return;
        ...
        function buildDepositTabs(bankListTo, gatewayList) {
            ...
            // Gateway tabs — first (one per gateway)
            if (gatewayList) {
            ...
            // Bank tab — after the gateway tabs
            $('#rowDepositTabs').append('<div class="deposit-tab-btn" id="btnTabBank">Bank</div>');
            ...
            // Activate the first tab by default (gateway if present, otherwise Bank)
            $('#rowDepositTabs .deposit-tab-btn').first().trigger('click');
        }

        function submitGatewayDeposit() {
            if (gatewayInFlight) return;
            gatewayInFlight = true;
            $('#btnGatewaySubmit').addClass('themeBtnDisabled');
        ...
        function endGatewaySubmit() {
            gatewayInFlight = false;
            $('#btnGatewaySubmit').removeClass('themeBtnDisabled');
        }
        ...
        function onSetGatewaySuccess(result, errCode, errText) {
            ...
            $.ajax({
                ...
                success: function (gwResponse) {
                    ...
                    // Show the row first: renderQr sizes the QR from the width of the
                    // cell it lands in, which is only measurable once the row is visible.
                    $('#rowGatewayQr').show();
                    renderQr(document.getElementById("gatewayQrCode"), qrContent);
                    gatewayQrActive = true;
                    $('#rowGatewaySubmit').hide();
                    $('#rowGatewayCheck').show();
                    endGatewaySubmit();
        ...
        function submitGatewayReject(qrId, gatewayAgentId) {
            if (rejectInFlight) return;
            if (!confirm('Are you sure you want to reject this pending deposit?')) return;
            rejectInFlight = true;
            $('#btnDepositReject').addClass('themeBtnDisabled');

            var data = { qrId: qrId, gatewayAgentId: gatewayAgentId };
            api.POST('/api/setGatewayReject', JSON.stringify(data), onSetRejectSuccess, onSetRejectFail);
        }

        function endRejectSubmit() {
            rejectInFlight = false;
            $('#btnDepositReject').removeClass('themeBtnDisabled');
        }

        function onSetRejectSuccess(result, errCode, errText) {
            var payload = $.parseJSON(result);
            var url = payload.endpoint + '/rejectran';
            $.ajax({
                type: 'POST',
                url: url,
                data: payload.encBody,
                ...
                success: function (gwResponse) {
                    ...
                    $('.tdDepositCheck').hide();
                    $('#rowPendingQr').hide();
                    $('#txtDepositStatus').html('Rejected');
                    translate();
                },
                error: function () {
                    alert('Failed to connect to payment gateway. Please try again.');
        ...
        function onSetRejectFail(errCode, errText) {
            alert(errText || 'Request failed');
            ShowAjaxError(errText || 'Request failed');
            endRejectSubmit();
        }
        ...
        function getDepositSuccess(result, errCode, errText) {
            ...
            if (!json.PendingRequest.IsPending) {
                ...
                // Show QR code if QrContent is available
                if (json.PendingRequest.QrContent && json.PendingRequest.QrContent.length > 0) {
                    $('#rowPendingQr').show();
                    renderQr(document.getElementById("pendingQrCode"), json.PendingRequest.QrContent);
                } else {
                    $('#rowPendingQr').hide();
                }

                // Show Check Status and Reject buttons only for gateway deposits
                if (rejectShowTimer) { clearTimeout(rejectShowTimer); rejectShowTimer = null; }
                if (json.PendingRequest.QrId && json.PendingRequest.QrId.length > 0) {
                    ...
                    var qrId = json.PendingRequest.QrId;
                    var gatewayAgentId = json.PendingRequest.FirePayGateAgentId;
                    $('#btnDepositReject').off('click').on('click', function () {
                        submitGatewayReject(qrId, gatewayAgentId);
                    });

                    // Reject button appears only 5 min after deposit create time
                    var REJECT_WAIT_SEC = 300;
                    var secsSinceCreate = json.PendingRequest.SecondsSinceCreate || 0;
                    if (secsSinceCreate >= REJECT_WAIT_SEC) {
                        $('#btnDepositReject').show();
                    } else {
                        $('#btnDepositReject').hide();
                        rejectShowTimer = setTimeout(function () {
                            $('#btnDepositReject').show();
                        }, (REJECT_WAIT_SEC - secsSinceCreate) * 1000);
                    }
                } else {
                    $('#btnDepositCheck').hide();
                    $('#btnDepositReject').hide();
        ...
        function setDeposit() {
            ...
            $('#btnDepositSubmit').addClass('themeBtnDisabled');
        ...
        function endDepositSubmit() {
            depositInFlight = false;
            $('#btnDepositSubmit').removeClass('themeBtnDisabled');
        }
        ...
        // Draw a QR the way scanners want it: every module a whole number of device pixels
        // wide, a 4-module quiet zone, and the canvas painted 1:1 so the phone does not
        // rescale and blur it. Fractional module sizes make the library round each module
        // up, which distorts the grid and some decoders then fail outright.
        // tools/qr-verify checks every rule this function has to keep.
        function qrModuleCount(text) {
            try {
                return new utils.QRCode(document.createElement('div'), {
                    text: text, width: 100, height: 100, quietZone: 0,
                    correctLevel: utils.QRCode.CorrectLevel.M
        ...
        function renderQr(el, text) {
            ...
            // Use the width the QR actually has in its cell. Falls back to the viewport
            // when the row is still hidden and therefore cannot be measured.
            var avail = el.clientWidth || (el.parentNode ? el.parentNode.clientWidth : 0);
            if (!avail) avail = window.innerWidth - 100;
            var targetCssPx = Math.max(180, Math.min(300, avail - 4));
            var count = qrModuleCount(text);
            if (!count) {   // library internals changed - fall back to a plain render
                new utils.QRCode(el, {
                    text: text, width: targetCssPx, height: targetCssPx, quietZone: 16,
                    correctLevel: utils.QRCode.CorrectLevel.M
                });
                return;
            }
            // Largest whole-pixel module size that still fits the space we have.
            var scale = Math.max(2, Math.floor((targetCssPx * dpr) / (count + 8)));
            var size = count * scale, qz = 4 * scale;
            new utils.QRCode(el, {
                text: text, width: size, height: size, quietZone: qz,
                correctLevel: utils.QRCode.CorrectLevel.M
            });
            var canvas = el.querySelector('canvas');
            var css = Math.floor((size + 2 * qz) / dpr);
            canvas.style.width = css + 'px';
            canvas.style.height = css + 'px';
        }
        function setQrCode(qrCodeData) {
            renderQr(document.getElementById("bnkQrCode"), qrCodeData);
        }
