...
function checkIframe() {
    var iframe = 'off';
    try { iframe = document.querySelector('siteapi[iframe]').getAttribute('iframe').toLowerCase(); } catch (err) { }
    if (iframe == 'off') {
...
function isMobile() {
    ...
    var mobile = "auto";
    try {
        mobile = decodeURIComponent((new RegExp('[?|&]mobile=([^&;]+?)(&|#|;|$)').exec(location.search) || [, ""])[1].replace(/\+/g, '%20')) || document.querySelector('siteapi[mobile]').getAttribute('mobile') || 'auto';
    } catch (err) { }
