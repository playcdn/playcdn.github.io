		...
		function gameDetail() {
			...
			this.Success = function (result, errCode, errText) {
				...
				for (i = 0; i < json.length - 1; i++) {
					var stake = utils.Money(parseInt(json[i].amt), 2);
					var com = utils.Money(parseInt(json[i].com), 2);
					var wl = utils.Money(parseInt(json[i].win), 2);
					var wl_style = json[i].win < 0 ? 'color:red' : '';
					if (json[i].id != "-1") {
						var date = json[i].ref + '<br />' + json[i].date;
						var detail = '<span class="bold">' + json[i].title + '</span><br />' + json[i].desc;
						var resultStatus = "";
						...
						var result = json[i].res;
						//if (gdpage == 1)
						$('#tblStatementGameRounds > tbody').append('<tr><td>' + (i + 1 + 100*(gdpage-1)) + '</td><td class="td-left">' + date + '</td><td class="td-left">' + detail + '</td><td class="td-md">' + result + '</td><td>' + stake + '</td><td>' + com + '</td><td class="bd" style="' + wl_style + '">' + wl + '</td></tr>');
					} else {
						$('#tblStatementGameRounds > tbody').append('<tr><td></td><td></td><td></td><td></td><td>' + stake + '</td><td>' + com + '</td><td class="bd" style="' + wl_style + '">' + wl + '</td></tr>');
					}
				}
				//paging
				var total = json[json.length - 1].total;
				var size = json[json.length - 1].size;
