(function (define) {
	define(["jquery", "translate", "utils", "aes-handler"], function ($, translate, utils, aes) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					try {
						...
						if(errCode == 1003 && action == '/api/logout')
							utils.Logout();
						else 
		...
		function ShowAjaxError(errCode, errText) {
			if(errCode == 1003)
				$('.errMsg').html(errText +  "<a href='#' onclick='location.reload();' style='padding-left:10px'><img src='/core/Images/icons/btn_refresh_on.png'  style='cursor:pointer;margin-bottom: -3px;' /> Reload </a>");
			else
				$('.errMsg').html(errText);
		...
		function ShowApiError(errCode, errText) {
			if(errCode == 1003)
				$('#errorBox').html(errText + "<a href='#' onclick='location.reload();' style='padding-left:10px'><img src='/core/Images/icons/btn_refresh_on.png'  style='cursor:pointer;margin-bottom: -3px;' /> Reload </a>").css('display', 'block');
			else
				$('#errorBox').html(errText).css('display', 'block');
		}
		function ShowApiException(errCode, errText, errResult) {
			if(errCode == 1003)
				$('#errorBox').html(errCode + ": " + errText + "<a href='#' onclick='location.reload();' style='padding-left:10px'><img src='/core/Images/icons/btn_refresh_on.png'  style='cursor:pointer;margin-bottom: -3px;' /> Reload </a><br\>" + errResult).css('display', 'block');
			else
