...
function checkIframe() {
    var iframe = 'off';
    try { iframe = document.querySelector('script[iframe]').getAttribute('iframe').toLowerCase(); } catch (err) { }
    if (iframe == 'off') {
...
function isMobile() {
    ...
    var mobile = "auto";
    try {
        mobile = decodeURIComponent((new RegExp('[?|&]mobile=([^&;]+?)(&|#|;|$)').exec(location.search) || [, ""])[1].replace(/\+/g, '%20')) || document.querySelector('script[mobile]').getAttribute('mobile') || 'auto';
    } catch (err) { }
...
if (allow && !isMobile()) {
	...
	require(['jquery', 'login', "/info/site?noext", 'translate', 'utils'], function ($, login, info, translate, utils) {
		...
		$(document).ready(function () {
			...
			$('body').prepend('<div style="width:100%;position:fixed;top:0;left:0;text-align:center;z-index:999"><div id="login_loading" class="loading"><div style="display:inline-block"><div style="margin-right:5px;width:16px;height:16px;"><img src="/core//Images/loading.gif" /></div><div style="color:#000">loading...</div></div></div></div>');
			...
			//var sessionStorageRef = sessionStorage.getItem("ref");
			//var urlRef = utils.GetCurrentUrlParam('ref');
			//if ((sessionStorageRef == null || sessionStorageRef == "null") && urlRef != null)
