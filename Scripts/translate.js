define(["utils", "jquery"], function (utils, $) {
	return function (code) {
		...
		switch (pageLang) {
			case "zh-cn": require([path + "/gb.js?v=" + version], function (gb) { translatePage(gb) }, function (err) { error(err); }); break;
			case "th": require([path + "/th.js?v=" + version], function (th) { translatePage(th) }, function (err) { error(err); }); break;
			case "vi": require([path + "/vi.js?v=" + version], function (vi) { translatePage(vi) }, function (err) { error(err); }); break;
			case "id": require([path + "/id.js?v=" + version], function (id) { translatePage(id) }, function (err) { error(err); }); break;
		}
		...
		this.warning = function (errText, errCode, failFunc, ShowError) {
			var pageLang = code ? code : utils.GetCookie('lang') || utils.GetCurrentUrlParam('lang') || "en-us";
			switch (pageLang) {
				case "zh-cn": require([path + "/gb.js?v=" + version], function (gb) { doTranslate(failFunc, errCode, ShowError, gb) }, function (err) { error(err); }); break;
				case "th": require([path + "/th.js?v=" + version], function (th) { doTranslate(failFunc, errCode, ShowError, th) }, function (err) { error(err); }); break;
				case "vi": require([path + "/vi.js?v=" + version], function (vi) { doTranslate(failFunc, errCode, ShowError, vi) }, function (err) { error(err); }); break;
				case "id": require([path + "/id.js?v=" + version], function (id) { doTranslate(failFunc, errCode, ShowError, id) }), function (err) { error(err); }; break;
				default: require([path + "/eng.js?v=" + version], function (eng) { doTranslate(failFunc, errCode, ShowError, eng) }, function (err) { error(err); }); break;
			}
